...
void LinearEncoder::initialize(uint8_t filter_count, uint8_t filter_sample_period)
{
    setInitConfig();
...
{
    return ((float)read()) * encoder_slope + end_stop_position;
}
...
void LinearEncoder::print_position_change(int encoder_num)
{
    ...
    if (_position_raw != _prev_position_raw)
    {
        ...
        Serial.print(((float)_position_raw) * encoder_slope + end_stop_position);
        Serial.println(" mm");
